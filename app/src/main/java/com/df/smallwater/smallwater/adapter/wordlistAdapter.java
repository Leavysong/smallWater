...
public class wordlistAdapter extends BaseQuickAdapter<Title, BaseViewHolder> {


Context context;

    public wordlistAdapter(int layoutResId, @Nullable List<Title> data , Context c) {
    ...
    protected void convert(BaseViewHolder helper, Title item) {
        ...
        helper.setText(R.id.tv_word_title,"第"+(helper.getPosition()+3)+"课");

        helper.setText(R.id.tv_word_content,"学英文"+item.getTitle());

        ImageView iv = new ImageView(context);

        iv = helper.getView(R.id.iv_word_img);
